<?php snippet('head') ?>
<body>
    <div class="detail default-detail">
    <a class="back-btn" href="javascript:window.history.back();">
        <div class="btn"></div>
    </a>
        <div class="detail--slider" id="sliderout">
            <?php
            if ($page->oneimgtoggle()->toBool() === false): ?>
            <div class="slider__btn">
                <div class="prev" onclick="prevscrolling()"><div class="btn"></div></div>
                <div class="next" onclick="nextscrolling()"><div class="btn"></div></div>
            </div>
            <?php elseif ($page->oneimgtoggle()->toBool() === true):?>
            <?php endif ?>
            <div class="slider__img sliderin" id="sliderin"><?= $page->workImgs()->toBlocks() ?></div>
        </div>
        <h1 class="detail--title"><?= $page->title()->html() ?></h1>
        <div class="detail--des"><?= $page->workDescription()->kt() ?></div>
        <div class="detail--info">
        <div class="detail--info__etc">
            <?php if ($page->worklocatin()->isNotEmpty()): ?>
                <em><?= $page->worklocatin()->html() ?></em>
            <?php endif ?>
                <em><?= $page->timeline()->html() ?></em>
            <?php if ($page->workCredit()->isNotEmpty()): ?>
                <em><?= $page->workCredit()->kt() ?></em>
            <?php endif ?>
        </div>
        <?php if ($page->workCaption()->isNotEmpty()): ?>
            <em class="detail--info__caption"><?= $page->workCaption()->kt() ?></em>
        <?php endif ?>
        </div>
    </div>
